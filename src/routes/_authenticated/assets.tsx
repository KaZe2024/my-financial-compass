          ...
          <table className="w-full min-w-[800px] text-sm">
            <thead className="text-left font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
              <tr>
                ...
                <th className="px-4 py-2 text-right">Δ</th><th className="px-4 py-2">Statut</th><th className="px-4 py-2 w-24"></th>
              </tr>
            </thead>
            <tbody>
              {visible.map((a: any) => {
                ...
                return (
                  <tr key={a.id} className={`border-t border-border/60 ${a.archived ? "opacity-50" : ""}`}>
                    ...
                    <td className="px-2 py-2 text-right"><RowActions table="assets" id={a.id} archived={a.archived} onEdit={() => setEditing(a)} linkedTxId={a.linked_transaction_id} /></td>
                  </tr>
                );
              })}
              {visible.length === 0 && <tr><td colSpan={8} className="px-4 py-10 text-center text-sm text-muted-foreground">Aucun actif</td></tr>}
            </tbody>
          </table>
        </div>
      </Panel>
